...
public class Person {
    ...
    public String toString() {
        String ageStr = age.isPresent() ? String.valueOf(age.getAsInt()) : "неизвестен";

        String addressStr = address.orElse("неизвестен");

        StringBuilder sb = new StringBuilder();

        sb.append(name).append(" ").append(surname).append(" (").append("возраст=").append(ageStr).append(", адрес=").append(addressStr).append(")");
    ...
    private void setAge(Integer age) {
        this.age = (age != null && age >= 0) ? OptionalInt.of(age) : OptionalInt.empty();
    }
    ...
    public void setAddress(String address) {
        this.address = (address != null && !address.isEmpty()) ? Optional.ofNullable(address) : Optional.empty();
    }
